      ...
      <div className="container mx-auto px-4 -mt-16 relative z-20 pb-24">
        <div className="flex flex-col lg:flex-row gap-12">
            <div className="w-full lg:w-2/3 space-y-12">
                <div className="bg-white p-8 md:p-16 rounded-[3.5rem] shadow-2xl border border-gray-100">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
                      <div>
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-primary mb-8 tracking-tighter italic uppercase">Equipe {titleName}.</h2>
                        <p className="text-gray-500 mb-10 leading-relaxed font-medium">
                          Nossa base operacional em <strong>{titleName}</strong> atende chamados críticos em tempo recorde. 
                          Oferecemos visita técnica com orçamento digital para sua maior comodidade.
                        </p>
                ...
                <div className="bg-primary text-white p-12 rounded-[3.5rem] shadow-2xl relative group overflow-hidden">
                   <div className="absolute inset-0 bg-accent opacity-0 group-hover:opacity-5 transition-opacity duration-700"></div>
                   <h3 className="text-3xl md:text-5xl font-black mb-10 tracking-tighter uppercase italic">Emergência em {titleName}?</h3>
                   <div className="flex flex-wrap gap-6">
                      <a href={CONTACT_INFO.whatsappLink} className="bg-accent text-primary px-10 py-5 rounded-2xl font-black uppercase text-sm shadow-xl flex items-center gap-4 italic">
                        <i className="fab fa-whatsapp text-2xl"></i> WHATSAPP 24H
                      </a>
                      <a href={CONTACT_INFO.phoneLink} className="bg-white/5 border border-white/20 px-10 py-5 rounded-2xl font-black uppercase text-sm flex items-center gap-4 italic">
                        <i className="fas fa-phone-alt"></i> CHAMAR AGORA
                      </a>
                   </div>
                </div>

                <FAQ items={GENERAL_FAQ} title={`FAQ ${titleName}`} />
            </div>

            <div className="w-full lg:w-1/3 space-y-8">
                <div className="sticky top-28 space-y-8">
                  <div className="bg-white p-2 rounded-[3rem] shadow-2xl border border-gray-100">
                    <ContactForm />
                  </div>
                  <div className="bg-gray-50 p-10 rounded-[3rem] border border-gray-100">
                    <h5 className="font-black text-primary mb-8 text-[10px] uppercase tracking-widest italic">Vizinhos Próximos</h5>
                    <div className="flex flex-wrap gap-2">
                       {BAIRROS.slice(0, 15).map((b, i) => (
                         <Link key={i} to={`/bairro/${b.toLowerCase().replace(/ /g, '-')}`} className="text-[9px] font-black text-gray-400 hover:text-accent transition-colors bg-white px-4 py-2 rounded-xl uppercase">
                           {b}
                         </Link>
                       ))}
                    </div>
